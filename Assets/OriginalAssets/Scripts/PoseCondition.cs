...
public class PoseCondition : MonoBehaviour
{
    ...
    void OnAnimatorIK()
    {
        ...
        if(origin == null)
        {
            ...
            TargetPos = autoGuidance.ModelPositions[Math.Min(autoGuidance.GuidanceTime + frame, readFileRowCount - 1)] + offset;
            TargetRot = autoGuidance.ModelQuaternions[Math.Min(autoGuidance.GuidanceTime + frame, readFileRowCount - 1)];
        }
        else
        {
            TargetPos = origin.TransformPoint(autoGuidance.ModelPositions[Math.Min(autoGuidance.GuidanceTime + frame, readFileRowCount - 1)]);
            TargetRot = origin.rotation * autoGuidance.ModelQuaternions[Math.Min(autoGuidance.GuidanceTime + frame, readFileRowCount - 1)];
